...
- [Module output](#module-output)
  ...
  - [ORF Caller step](#orf-caller-step) - Identify protein-coding genes (ORFs) with an ORF caller
    ...
    - [MetaEuk](#metaeuk) - Output from MetaEuk (optional)
  - [Protein consolidation](#protein-consolidation) - Protein clusters across ORF callers and contigs
  ...
  - [Functional and taxonomic annotation](#functional-and-taxonomical-annotation) - Predict the function and the taxonomy of ORFs
...
[BBduk](https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/bbduk-guide/) is a filtering tool that removes specific sequences from the samples using a reference fasta file.
...
[BBnorm](https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/bbnorm-guide/) is a tool from BBmap that allows to reduce the coverage of highly abundant sequence kmers and remove sequences representing kmers that are below a threshold.
...
- `megahit/`
  - `megahit_assembly.log`: log file of Megahit run.
...
[QUAST](https://github.com/ablab/quast) computes assembly statistics (contig counts, N50, GC content etc.), also shown in the MultiQC report.

<details markdown="1">
<summary>Output files</summary>

- `quast/`
  - `<assembly_name>.tsv`: summary statistics.
  - `<assembly_name>/`: full QUAST report, including `report.html` and plots.
...
Each active caller's output below is produced independently, under its own `<caller>` name.
Calls for the same gene are consolidated across callers in the [protein consolidation](#protein-consolidation) and [summary tables](#summary-tables) output.
...
N.B. Prodigal or Prokka are recommended for prokaryotic samples.
...
Another eukaryote-targeted alternative is [MetaEuk](https://github.com/soedinglab/metaeuk), a splice-aware ORF caller that aligns contigs against a reference protein database (`--metaeuk_db`, or downloaded automatically) and can call a single gene model spanning an intron.
...
- `metaeuk/`
  - `<assembly_name>.metaeuk_format.faa.gz`: ORFs in amino acid format fasta file
  - `<assembly_name>.metaeuk_format.gff.gz`: CDS-only genome feature file, with a single shared ID per (possibly multi-exon) gene, used for quantification

</details>

### Protein consolidation

Proteins from all ORF callers are clustered with [MMseqs2](https://github.com/soedinglab/MMseqs2), so that one gene called on different contigs, or by different callers, can be counted and annotated once.
See [Consolidating calls for the same gene](usage.md#consolidating-calls-for-the-same-gene).
Skipped with `--skip_protein_consolidation`.

<details markdown="1">
<summary>Output files</summary>

- `protein_consolidate/`
  - `<assembly_name>.protein_consolidate_<identity>.clusters.tsv`: cluster membership, one row per ORF, with the cluster representative.
  - `<assembly_name>.protein_consolidate_<identity>.representatives.fa.gz`: amino acid sequences of the cluster representatives.
...
- `summary_tables/`
  ...
  - `<assembly_name>.locus_consolidate.counts.tsv.gz` and `<assembly_name>.protein_consolidate_<identity>.counts.tsv.gz`: consolidated counts, see [Summary tables](#summary-tables).
...
- `eukulele/<assembly_name>.<orfcaller_name>.<database>_<database>/`
  - `mets_full/diamond/proteins.diamond.out.gz`: EUKulele's raw Diamond alignments, only written with `--save_eukulele_alignments`
  - `taxonomy_counts/*.csv.gz`: counts for different ranks, see [EUKulele documentation](https://eukulele.readthedocs.io)
  - `taxonomy_estimation/proteins-estimated-taxonomy.out.gz`: EUKulele taxonomy assignment
- `summary_tables/`
  - `<assembly_name>.<orfcaller_name>.<database>.eukulele.taxonomy.tsv.gz`: reformatted EUKulele output
...
algorithm.
A GTDB database is provided, and you can build your own, see the [usage documentation](usage.md#taxonomic-annotation-with-diamond).
...
- `summary_tables/`
  ...
  - `<assembly_name>.<orfcaller_name>.<database>.eukulele.taxonomy.tsv.gz`: taxonomic annotation per ORF for specific database.
