...
- [#NN](https://github.com/nf-core/metatdenovo/pull/NN) - Update the vendored `bbmap/bbnorm` module to fix it failing under Singularity/Apptainer when the host's `$TMPDIR` isn't bind-mounted into the container (the same bug patched directly on `master` for the 1.4.1 release, now picked up here via the proper upstream module update), addresses [#516](https://github.com/nf-core/metatdenovo/issues/516) (@erikrikarddaniel)
