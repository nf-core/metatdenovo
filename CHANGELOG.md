...
- [#NN](https://github.com/nf-core/metatdenovo/pull/NN) - Add `--save_parquet` to also write every `summary_tables/` tsv as Parquet, alongside the default gzipped TSV, using the shared `nf-core/modules` component `duckdb/table2parquet` (ported from nf-core/magmap). Off by default; the TSVs are unaffected either way, addresses [#473](https://github.com/nf-core/metatdenovo/issues/473) (@erikrikarddaniel)
