...
- [#NN](https://github.com/nf-core/metatdenovo/pull/NN) - Replace TransRate with QUAST for assembly quality statistics: TransRate is essentially unmaintained (no meaningful activity since ~2017), and its multithreading is inert as used here (only its read-mapping stage benefits, which never runs since this pipeline calls it reference-free), addresses [#487](https://github.com/nf-core/metatdenovo/issues/487) (@erikrikarddaniel)
