...
- [#504](https://github.com/nf-core/metatdenovo/pull/504) - Add each `Unassigned_*` category from featureCounts' summary (`Unassigned_NoFeatures`, `Unassigned_Ambiguity`, etc.) as its own column in `<assembly>.<caller>.overall_stats.tsv.gz`, alongside the existing `n_post_trimming`/`idxs_n_mapped`/etc. columns, so reads mapped but not counted are easier to diagnose without opening individual per-sample `.summary` files, addresses [#451](https://github.com/nf-core/metatdenovo/issues/451) (@erikrikarddaniel)
