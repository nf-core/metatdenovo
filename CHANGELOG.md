...
- [#NN](https://github.com/nf-core/metatdenovo/pull/NN) - Add `--bbmap_ambiguous` (`best`/`all`/`random`/`toss`, default `best`) and `--featurecounts_fraction` to control how BBMap and featureCounts handle reads that align to more than one site, addresses [#464](https://github.com/nf-core/metatdenovo/issues/464) (@erikrikarddaniel)
