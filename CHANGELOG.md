...
- [#NN](https://github.com/nf-core/metatdenovo/pull/NN) - Give `BEDTOOLS_SORT` its own `conf/modules.config` memory/time override (scaling with `task.attempt`, not cpus, since it's single-threaded) instead of the vendored `process_single` label's 6GB/4h, which is sized for small/test-scale input; on a real production run the combined, not-yet-consolidated BED across all active ORF callers can be two orders of magnitude larger and OOM-kills the process even after its automatic retry (@erikrikarddaniel)
