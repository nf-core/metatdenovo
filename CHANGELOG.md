...
- [#NN](https://github.com/nf-core/metatdenovo/pull/NN) - Add `--annotate_only_consolidated` (default `true`): once protein consolidation clusters multiple `--orf_caller`/`--user_orfs` sources' proteins, only annotate the cluster representatives with EGGNOG/KOFAMSCAN/DBCAN/EUKULELE/DIAMOND_TAXONOMY/HMMCLASSIFY instead of every individual source's full protein set as well, avoiding up to N-fold redundant annotation of genes multiple sources agree on. No effect when only one ORF source is active or consolidation is skipped (@erikrikarddaniel)
