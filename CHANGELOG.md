...
- [#504](https://github.com/nf-core/metatdenovo/pull/504) - Add each `Unassigned_*` category from featureCounts' summary (`Unassigned_NoFeatures`, `Unassigned_Ambiguity`, etc.) as its own column in every `<assembly>.<caller>.overall_stats.tsv.gz`, alongside the existing `n_post_trimming`/`idxs_n_mapped`/etc. columns, so reads mapped but not counted are easier to diagnose without opening individual per-sample `.summary` files. Not purely additive: because the new columns sort before existing ones, the caller-named count column is no longer directly after `idxs_n_unmapped` -- read columns by name, addresses [#451](https://github.com/nf-core/metatdenovo/issues/451) (@erikrikarddaniel)
- [#502](https://github.com/nf-core/metatdenovo/pull/502) - Automatically download and build a MetaEuk reference database (`--metaeuk_db_name`, default `UniRef50`, cached under `--metaeuk_db_dir`) when `--metaeuk_db` isn't set, matching KofamScan's existing auto-download pattern, addresses [#485](https://github.com/nf-core/metatdenovo/issues/485) (@erikrikarddaniel)
- [#501](https://github.com/nf-core/metatdenovo/pull/501) - Add `--annotate_only_consolidated` (default `true`): once protein consolidation clusters multiple `--orf_caller`/`--user_orfs` sources' proteins, only annotate the cluster representatives with EGGNOG/KOFAMSCAN/DBCAN/EUKULELE/DIAMOND_TAXONOMY/HMMCLASSIFY instead of every individual source's full protein set as well, avoiding up to N-fold redundant annotation of genes multiple sources agree on. No effect when only one ORF source is active or consolidation is skipped (@erikrikarddaniel)
- [#494](https://github.com/nf-core/metatdenovo/pull/494) - Make user-supplied ORFs (`--user_orfs_gff`/`--user_orfs_faa`, and a new `--user_orfs` CSV supporting multiple named sets, schema `assets/schema_user_orfs.json`) additive with `--orf_caller` instead of mutually exclusive with it, and let them participate in locus/protein consolidation like any other caller (@erikrikarddaniel)
...
- [#527](https://github.com/nf-core/metatdenovo/pull/527) - `KOFAMSCAN_DOWNLOAD` and `EGGNOG_DOWNLOAD` now stage their reference-database URLs via Nextflow's own `file()` instead of shelling out to `wget`, so the download works even when the compute node running the task has no network access itself; also fixes `EGGNOG_DOWNLOAD`'s download host, which had silently been pointing at a dead URL (`eggnog6.embl.de` instead of `eggnog5.embl.de`), addresses [#365](https://github.com/nf-core/metatdenovo/issues/365) (@erikrikarddaniel)
...
- [#528](https://github.com/nf-core/metatdenovo/pull/528) - Rename `docs/large_datasets.md` to `docs/usage_large_datasets.md` so the "Coping with large datasets" page is actually published on the website. The nf-core website only builds a page for a docs file whose path contains `usage` or `output`, so the page existed on GitHub but returned 404 on nf-co.re, and the six links to it from `docs/usage.md` -- the pipeline's guidance on assembly memory problems -- were all dead there, addresses [#493](https://github.com/nf-core/metatdenovo/issues/493) (@erikrikarddaniel)
- [#525](https://github.com/nf-core/metatdenovo/pull/525) - Bundle `awscli` alongside `EGGNOG_DOWNLOAD`, `KOFAMSCAN_DOWNLOAD` and `RUNDBCAN_DATABASE`'s own tools in a combined Wave container each, and re-enable dbCAN in `-profile test_full`. These three processes cache their downloaded database in an `s3://` `storeDir`, and Nextflow's AWS Batch executor stages that copy by shelling out to `aws` from inside the task's own container -- which none of the three had, so the download succeeded and the run then died copying it into the store (`aws: command not found`). Also fixes a related bug: `workflows/metatdenovo.nf`'s `eukulele_dbpath` existence check used `java.io.File`, which can't resolve an `s3://` path and silently created a bogus local `s3:` directory instead; switched to Nextflow's own `file()`. KOfamScan works the same way but is left skipped in `-profile test_full` for now, out of scope here, addresses [#471](https://github.com/nf-core/metatdenovo/issues/471) (@danilodileo)
- [#524](https://github.com/nf-core/metatdenovo/pull/524) - Update the vendored `bbmap/bbnorm` module to fix it failing under Singularity/Apptainer when the host's `$TMPDIR` isn't bind-mounted into the container (the same bug patched directly on `master` for the 1.4.1 release, now picked up here via the proper upstream module update), addresses [#516](https://github.com/nf-core/metatdenovo/issues/516) (@erikrikarddaniel)
- [#523](https://github.com/nf-core/metatdenovo/pull/523) - Regenerate `tests/stub.nf.test.snap`, the one snapshot #511 missed after bumping `nft-utils` to 1.2.0, fixing a spurious `Different Snapshot` CI failure, addresses [#522](https://github.com/nf-core/metatdenovo/issues/522) (@erikrikarddaniel)
- [#521](https://github.com/nf-core/metatdenovo/pull/521) - Give `BBMAP_ALIGN`, `BBMAP_BBDUK` and `BBMAP_INDEX` the same JVM memory headroom multiplier `BBMAP_BBNORM` already carries (`-Xmx` at 90% of `task.memory` instead of the exact ceiling), leaving room for JVM overhead and BBMap's own off-heap hashing structures below the container's hard memory limit. Previously only BBNorm was patched this way, despite all four tools sharing the same unguarded `-Xmx${task.memory.toGiga()}g` pattern; addresses [#520](https://github.com/nf-core/metatdenovo/issues/520) (@danilodileo)
- [#519](https://github.com/nf-core/metatdenovo/pull/519) - Gate the `.*SAMTOOLS.*` block's `*.bam`/`*.bam.bai` publishDir rules on `--save_bam`, matching `BBMAP_ALIGN`'s own `*.bam` rule and the param's description ("Save the bam files from mapping"). Previously, turning `--save_bam` off only suppressed BBMap's unsorted BAM -- a sorted copy of the same alignments (plus its index) still got published unconditionally via samtools, several GB per full-test run. Also documents the `samtools/` output directory, addresses [#474](https://github.com/nf-core/metatdenovo/issues/474) (@danilodileo)
- [#509](https://github.com/nf-core/metatdenovo/pull/509) - Fix `--user_orfs_gff`/`--user_orfs_faa` silently collapsing every locus a re-supplied raw MetaEuk gff/fasta pair is the sole contributor to into one, by normalising it the same way a pipeline-internal MetaEuk run already is; also make `FORMAT_GFF2BED`/`FORMAT_METAEUK_GFF` fail loudly instead of silently on an unparseable ID, closes [#508](https://github.com/nf-core/metatdenovo/issues/508) (@erikrikarddaniel)
- [#506](https://github.com/nf-core/metatdenovo/pull/506) - Fix five `-stub` bugs in the eggnog/eukulele/kofamscan/prokka annotation modules, only reachable with multiple ORF callers and every annotation tool active at once, addresses [#476](https://github.com/nf-core/metatdenovo/issues/476) (@erikrikarddaniel)
- [#503](https://github.com/nf-core/metatdenovo/pull/503) - `--skip_dbcan false`/`--skip_eggnog false`/`--skip_eukulele false`/`--skip_fastqc false`/`--skip_kofamscan false`/`--skip_protein_consolidation false`/`--skip_qc false`/`--skip_trimming false`/`--annotate_only_consolidated false`/`--min_contig_length` now actually work from the command line. Nextflow's CLI parser hands boolean/integer params through as strings, and `'false'` is truthy in Groovy, so a step meant to be turned back on from the CLI silently stayed skipped, addresses [#478](https://github.com/nf-core/metatdenovo/issues/478) (@erikrikarddaniel)
