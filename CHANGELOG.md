...
- Gate the `.*SAMTOOLS.*` block's `*.bam`/`*.bam.bai` publishDir rules on `--save_bam`, matching `BBMAP_ALIGN`'s own `*.bam` rule and the param's description ("Save the bam files from mapping"). Previously, turning `--save_bam` off only suppressed BBMap's unsorted BAM -- a sorted copy of the same alignments (plus its index) still got published unconditionally via samtools, several GB per full-test run. Also documents the `samtools/` output directory, addresses [#474](https://github.com/nf-core/metatdenovo/issues/474) (@danilodileo)
