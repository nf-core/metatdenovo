...
- [#523](https://github.com/nf-core/metatdenovo/pull/523) - Regenerate `tests/stub.nf.test.snap`, the one pipeline-level software-versions snapshot #511 missed regenerating after bumping `nft-utils` to 1.2.0 -- its deterministic inner-map key sorting means the stale, pre-1.2.0 content in that one file no longer matched what a live run actually produces, causing a spurious `Different Snapshot` CI failure on any unrelated PR unlucky enough to touch it, addresses [#522](https://github.com/nf-core/metatdenovo/issues/522) (@erikrikarddaniel)
