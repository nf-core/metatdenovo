...
- [#506](https://github.com/nf-core/metatdenovo/pull/506) - Add a `test_stub`/`tests/stub.nf.test` pipeline test that runs the whole default pipeline plus every annotation tool (EGGNOG/KOFAMSCAN/dbCAN/EUKulele/HMMER) under `-stub`, asserting one task per active ORF caller for each -- none of them need a real database in stub mode, and it is the one combination the per-tool test profiles can't afford to run for real, addresses [#476](https://github.com/nf-core/metatdenovo/issues/476) (@erikrikarddaniel)
...
- [#506](https://github.com/nf-core/metatdenovo/pull/506) - Fix several `-stub` bugs surfaced while building `test_stub` (none reachable from any other test profile, since none exercises multiple ORF callers with every annotation tool on at once): `eukulele/format_tax` and `prokka/gff2tsv` had no `stub:` block at all, so `-stub` ran their real R scripts against a real script's stubbed-out input and crashed; `kofamscan/unique`'s stub wrote a filename that didn't match its own declared output pattern; `eggnog/download`'s stub never created `eggnog_proteins.dmnd`, one of its four declared outputs; `tidyverse/stripcdsprefix`'s stub could collide with its own staged input's filename and refuse to overwrite it. Also picks up nf-core/modules#12798's fix for `seqkit/grep`'s stub appending `.gz` twice to gzipped input, addresses [#476](https://github.com/nf-core/metatdenovo/issues/476) (@erikrikarddaniel)
