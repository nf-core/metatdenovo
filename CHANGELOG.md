...
- [#NN](https://github.com/nf-core/metatdenovo/pull/NN) - `--skip_dbcan false`/`--skip_eggnog false`/`--skip_eukulele false`/`--skip_fastqc false`/`--skip_kofamscan false`/`--skip_protein_consolidation false`/`--skip_qc false`/`--skip_trimming false`/`--annotate_only_consolidated false`/`--min_contig_length` now actually work from the command line. Nextflow's CLI parser hands boolean/integer params through as strings, and `'false'` is truthy in Groovy, so a step meant to be turned back on from the CLI silently stayed skipped, addresses [#478](https://github.com/nf-core/metatdenovo/issues/478) (@erikrikarddaniel)
